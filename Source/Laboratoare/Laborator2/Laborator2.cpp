...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		{
			VertexFormat (glm::vec3(0, 0, 2), glm::vec3(0, 2, 0)),
			VertexFormat (glm::vec3(2, 0, 2), glm::vec3(0, 2, 0)),
			VertexFormat (glm::vec3(0, 2, 2), glm::vec3(0, 2, 2)),
			VertexFormat (glm::vec3(2, 2, 2), glm::vec3(0, 2, 2)),
			VertexFormat (glm::vec3(0, 0, 0), glm::vec3(2, 0, 0)),
			VertexFormat (glm::vec3(2, 0, 0), glm::vec3(2, 0, 0)),
			VertexFormat (glm::vec3(0, 2, 0), glm::vec3(0, 0, 2)),
			VertexFormat (glm::vec3(2, 2, 0), glm::vec3(0, 0, 2)),
			// TODO: Complete the information for the cube
		};
		//puncte piramida ex4
		vector<VertexFormat> verticesP
		{
			VertexFormat(glm::vec3(0, 0, 2), glm::vec3(0, 0, 1)),
		...
		//triunghiuri patrat ex6
		vector<unsigned short>indicesPatrat =
		{
			0,1,3,
			0,2,3,
		};
		//triunghiuri piramida ex4
		...
		Mesh *piramida = CreateMesh("piramida", verticesP, indicesP);
		Mesh* cube = CreateMesh("cube3", vertices, indices);
		Mesh *patrat = CreateMesh("patrat", verticesPatrat, indicesPatrat);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	//cub ex2
	RenderMesh(meshes["cube1"], shaders["VertexColor"], glm::vec3(-1, 0, 0), glm::vec3(0.25f));
	//cub ex3
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(1,0,0), glm::vec3(0.25f));
	//piramida ex4
	RenderMesh(meshes["piramida"], shaders["VertexColor"], glm::vec3(2, 0, 0), glm::vec3(0.25f));
	//patrat ex6
	RenderMesh(meshes["patrat"], shaders["VertexColor"], glm::vec3(3, 0, 0), glm::vec3(0.25f));
